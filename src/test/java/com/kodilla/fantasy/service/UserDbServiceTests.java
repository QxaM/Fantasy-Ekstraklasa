...
public class UserDbServiceTests {
    ...
    void shouldGetUsers() {
        //Given
        User user1 = new User(1L, "User 1", "user@test.com", new ArrayList<>(), new Squad(), 0);
        User user2 = new User(2L, "User 2", "user@test.com", new ArrayList<>(), new Squad(), 0);
    ...
    void shouldCreateSquad() throws ElementNotFoundException {
        ...
        User userWithSquad = new User(1L, "User 1", "user@test.com", new ArrayList<>(), squad, 0);
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
